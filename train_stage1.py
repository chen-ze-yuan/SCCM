...
optimizer = torch.optim.Adam(Encodermodel.parameters(), lr=0.001)
...
def dev(model, device, dev_loader, loss1):
    model.eval()
    total_loss = 0.0
    with torch.no_grad():
        for batchT in dev_loader:
            ...
            dev_loss = 0.5 * loss_py + 0.5 * loss_sy

            total_loss += dev_loss.item()
    avg_loss = total_loss / len(dev_loader)
    return avg_loss

def train_net(model, device, train_loader, dev_loader, optimizer, loss1, checkpoint__path):
    best_loss = float('inf')
    ...
    for epoch in range(start_epoch, 20):
        ...
        for batch in train_loader:
            ...
            train_loss = 0.5 * loss_py + 0.5 * loss_sy
        ...
        dev_loss = dev(model,  device, dev_loader, loss1)
...
if __name__ == '__main__':
    ...
    train_net(Encodermodel, device, train_loader, dev_loader, optimizer, ctc_loss, checkpoint_path)
